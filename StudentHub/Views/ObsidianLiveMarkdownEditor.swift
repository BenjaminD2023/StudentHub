    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        private func styleRenderedLine(
            ...
            if let checklist = firstMatch(#"^(\s*[-*+]\s+\[[ xX]\]\s+)(.*)$"#, in: line) {
                let marker = globalRange(checklist.range(at: 1), lineRange: range)
                let item = globalRange(checklist.range(at: 2), lineRange: range)
                let isChecked = line.range(of: "[x]", options: .caseInsensitive) != nil
                storage.addAttributes(
                    [.foregroundColor: UIColor.systemBlue, .font: UIFont.systemFont(ofSize: 15, weight: .semibold)],
                    range: marker
                )
                if isChecked {
                    storage.addAttributes(
